...
def find_nearby_therapists_by_location(location: str) -> str:
    """
    Finds therapists near a location using OpenStreetMap (no API key required).
    """
    try:
        ...
        return (
            ...
            f"You can search for therapists near you using this map:\n"
            f"https://www.openstreetmap.org/#map=14/{lat}/{lon}\n\n"
            "You may also search online for 'therapist near me' for local results."
        )

    except Exception:
        return (
            "I’m unable to look up nearby therapists right now, "
            "but I can still help you with emotional support or next steps."
        )
